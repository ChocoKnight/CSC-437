...
var MESSAGES = /* @__PURE__ */ ((MESSAGES2) => {
  MESSAGES2["noUsernamePassword"] = "Must provide username and password";
  MESSAGES2["invalidUsernamePassword"] = "Invalid username or password";
  MESSAGES2["userNameExists"] = "Username exists";
  return MESSAGES2;
})(MESSAGES || {});
...
function create(username, password) {
  return new Promise((resolve, reject) => {
    if (!username || !password) {
      reject("Must provide username and password" /* noUsernamePassword */);
    }
    credentialModel.find({ username }).then((found) => {
      if (found.length) reject("Username exists" /* userNameExists */);
    }).then(
...
function verify(username, password) {
  return new Promise((resolve, reject) => {
    credentialModel.find({ username }).then((found) => {
      ...
      else reject("Invalid username or password" /* invalidUsernamePassword */);
    }).then((credsOnFile) => {
      if (credsOnFile)
