...
var ERRORMESSAGES = /* @__PURE__ */ ((ERRORMESSAGES2) => {
  ERRORMESSAGES2["e401"] = "Unauthorized";
  ERRORMESSAGES2["e404"] = "Bad request: Invalid input data";
...
router.post("/register", (req, res) => {
  ...
  if (!username || !password) {
    res.status(400).send("Bad request: Invalid input data" /* e404 */);
  } else {
...
router.post("/login", (req, res) => {
  ...
  if (!username || !password) {
    ...
    import_credential_svc.default.verify(username, password).then((goodUser) => generateAccessToken(goodUser)).then((token) => res.status(200).send({ token })).catch((error) => res.status(401).send("Unauthorized" /* e401 */));
  }
});
